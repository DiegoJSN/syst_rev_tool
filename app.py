            ...
            if action == "import_studies":
                ...
                if wos and wos.filename:
                    if not wos.filename.lower().endswith(".xls"):
                        ...
                        try:
                            added, dupes = import_wos_xls(review_id, path)
                            inserted += added
                            duplicates += dupes
                ...
                if scopus and scopus.filename:
                    if not scopus.filename.lower().endswith(".csv"):
                        ...
                        try:
                            added, dupes = import_scopus_csv(review_id, path)
                            inserted += added
                            duplicates += dupes
                        except Exception as e:
                ...
                flash(
                    ...
                    f"{duplicates} duplicates were ignored. "
                    f"{deleted} empty rows were removed.",
                    "success",
                )
                return redirect(url_for("review_main", review_id=review_id))
