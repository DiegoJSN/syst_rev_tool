    ...
    def review_main(review_id: int):
        ...
        if request.method == "POST":
            ...
            if action == "import_studies":
                ...
                delete_empty_studies(review_id)
                current_duplicates = db.execute(
                    "SELECT duplicates_removed FROM review WHERE id = ?;",
                    (review_id,),
                ).fetchone()["duplicates_removed"]
                db.execute(
                    "UPDATE review SET duplicates_removed = ? WHERE id = ?;",
                    (current_duplicates + duplicates, review_id),
                )
                db.commit()
                refresh_cached_metrics(review_id)
                for e in errors:
                ...
                flash(
                    ...
                    f"{duplicates} duplicated studies were detected and removed.",
                    "success",
                )
                return redirect(url_for("review_main", review_id=review_id))
